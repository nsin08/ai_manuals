...
| Reliability Sprint increment: model-swappable config + optional vision ingestion + optional reranker + answer confidence surfaced | Codex | done | 2026-02-19 | `packages/application/config.py`, `packages/adapters/vision/*`, `packages/adapters/reranker/*`, `apps/api/main.py`, `apps/ui/main.py`, `scripts/run_*.py`, `pytest tests -q` |
| Reliability Sprint increment: background ingestion jobs + progress API + page-parallel ingestion | Codex | done | 2026-02-19 | `apps/api/ingestion_jobs.py`, new `/jobs/*` endpoints in `apps/api/main.py`, page workers in `packages/application/use_cases/ingest_document.py`, admin polling UI in `apps/ui/admin_main.py`, `tests/unit/test_ingest_parallel_progress.py`, `tests/integration/test_api_e2e.py`, `pytest tests -q` |
