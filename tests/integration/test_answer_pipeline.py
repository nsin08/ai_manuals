...
def test_answer_after_ingestion_returns_citations(tmp_path: Path) -> None:
    ...
    output = answer_question_use_case(
        AnswerQuestionInput(
            query='What does fault F005 mean and what corrective action is listed?',
            doc_id=doc.doc_id,
            top_n=5,
        ),
    ...
    assert output.status in {'ok', 'not_found'}
    assert output.citations
    assert all(c.doc_id == doc.doc_id for c in output.citations)
